...
load("data/unaggregated_table.RData")

# Generate and display the table with additional headers
# Generate and display the table with the revised caption
kable(result_unaggregated, format = "latex", booktabs = TRUE, 
      caption = "\\textbf{Conditional AIC Scores for SIRS Model with Simulated Data}. The columns represent the smoothing basis used to fit the model, while the rows indicate the basis used to generate the simulated data. The degrees of freedom are computed as the trace of the penalized smoothing matrix.") %>%
...
# Load the saved R object
# Load the data
load("data/aggregated_table_sim.RData")



# Generate and display the table with the revised caption
kable(transposed_result, format = "latex", booktabs = TRUE, 
...
# Load the saved R object
aic_table <- readRDS("data/Scarlet_combined_k9.rds")

# Generate and display the table with additional headers
kable(aic_table, format = "latex", booktabs = TRUE, 
      col.names = c("Smooth Type","cAIC"),
      caption = "\\textbf{Conditional AIC Scores of calibrating models with varying smoothing basis, calibrated to Ontario scarlet fever (1929-1930.} The degrees of freedom are defined as the model degrees of freedom which is computed as the trace of penalized smoothing matrix.") %>%
...
# Load the saved R object
aic_table <- readRDS("data/Ireland_combined_k7.rds")

# Generate and display the table with additional headers
kable(aic_table, format = "latex", booktabs = TRUE, 
      col.names = c("Smooth Type", "cAIC"),
      caption = "\\textbf{Conditional AIC Scores of calibrating models with varying smoothing basis, calibrated to Ireland Covid-19 (2020).} The degrees of freedom are defined as the model degrees of freedom which is computed as the trace of penalized smoothing matrix.") %>%
  kable_styling(latex_options = c("hold_position", "float_placement"))
```
